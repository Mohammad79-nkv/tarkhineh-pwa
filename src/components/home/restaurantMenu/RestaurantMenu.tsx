...
const RestaurantMenu = () => {
  const menuData = [
    ...
    { id: 2, name: "دسر", imageSrc: "/images/dissert.png", route: "/foods" },
    {
      id: 3,
      name: "پیش غذا",
      imageSrc: "/images/preFood.png",
      route: "/foods",
    },
    {
      id: 4,
      name: "غذای اصلی",
      imageSrc: "/images/mainFood.png",
      route: "/foods",
    },
  ];
        ...
        {menuData.map((item) => (
          ...
          >
            ...
            <button className="absolute bottom-[-15px] flex justify-center items-center bg-primary p-2 text-white shadow-cards-shadow rounded-4 w-20 md:w-24 h-8 text-xs md:text-base lg:h-12 lg:w-36 lg:text-xl lg:bottom-[-20px] hover:bg-Shade-2 transition-all">
              <Link href={`${item.route}?${item.items}`}>{item.name}</Link>
            </button>
          </div>
        ))}
      </div>
